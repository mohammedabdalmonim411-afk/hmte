...
if [[ ${#PROJECT_FILES[@]} -gt 0 || ${#EVIDENCE_FILES[@]} -gt 0 || ${#VERDICT_FILES[@]} -gt 0 ]]; then
...
" 2>/dev/null || echo "[]")
    ...
    VERIFICATION_RESULT=$(python3 - "$PROJECT_ROOT" "$PROJECT_FILES_JSON" "$EVIDENCE_FILES_JSON" "$VERDICT_FILES_JSON" <<'PY'
...
# --- 3. Project-plane files: need evidence ownership ---
if project_files:
    # Collect all claimed file paths from ALL evidence files
    claimed_files = set()
    ...
    for ev_path in glob.glob(f"{evidence_dir}/*.json"):
        try:
            ...
            for cf in ev.get('changed_files', []):
                claimed_files.add(cf)
            for af in ev.get('artifact_paths', []):
                claimed_files.add(af)
        except Exception:
            pass

    # Also collect claimable paths from .phase_control/lock.json,
    # README.md, HERMES.md etc (docs the user might reasonably modify)
    # → we only check files explicitly in changed_files/artifact_paths

    for pf in project_files:
        if pf not in claimed_files:
            violations.append(
...
./.phase_control/delegations/p0_critical_attempt_1_verifier.json
./.phase_control/delegations/p0_critical_attempt_1_worker.json
./.phase_control/errors/.gitkeep
./.phase_control/evidence/.gitkeep
./.phase_control/evidence/p0_critical_attempt_1.json
./.phase_control/goal_lock.json
./.phase_control/instructions/.gitkeep
...
./.phase_control/logs/.gitkeep
./.phase_control/logs/p0_critical_attempt_1.commands.jsonl
./.phase_control/phases.json
./.phase_control/pids/.gitkeep
...
./.phase_control/traces/.gitkeep
./.phase_control/verdicts/.gitkeep
./.phase_control/verdicts/p0_critical_attempt_1.json
