...
#!/bin/bash
# install-to-hermes.sh — Install HTE skill into Hermes profile
#
# Usage:
#   bash install-to-hermes.sh [--all|--profile NAME|--global|--verify-only] [--force]
#
# --all          Install to current profile + install deps + verify (default)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo ""
            echo "  --all          Install + deps + verify (default)"
            echo "  --profile NAME Install to specific Hermes profile"
            echo "  --global       Install to global skills directory"
            echo "  --verify-only  Only verify installation, don't copy files"
...
if [ "$MODE" = "global" ]; then
    TARGET_DIR="$HERMES_HOME/skills/hmte"
elif [ "$MODE" = "profile" ]; then
    TARGET_DIR="$HERMES_HOME/profiles/$PROFILE/skills/hmte"
else
    TARGET_DIR="$HERMES_HOME/profiles/$PROFILE/skills/hmte"
fi
...
if [ "$ERRORS" -gt 0 ]; then
    echo "╔══════════════════════════════════════════════╗"
    echo "  ⚠️  Installation completed with $ERRORS error(s)"
    echo "╚══════════════════════════════════════════════╝"
    exit 1
else
    echo "╔══════════════════════════════════════════════╗"
    echo "  ✅ HTE installed successfully!"
    echo "     Target: $TARGET_DIR"
    echo ""
    echo "  Next steps:"
    echo "    1. cd /path/to/your/project"
    echo "    2. cp -r /path/to/hmte/scripts ./scripts"
...
# =============================================================================
# Step 5: Get changed files since lock
# =============================================================================
CHANGED_FILES="$(git -C "$PROJECT_ROOT" diff --name-only "$LOCK_GIT_HEAD"..HEAD 2>/dev/null || true)"
...
log_info "Detected $CHANGED_COUNT changed file(s) since lock."
...
# =============================================================================
VERDICT_FILES="$(git -C "$PROJECT_ROOT" diff --name-only --diff-filter=M "$LOCK_GIT_HEAD"..HEAD -- ".phase_control/verdicts/" 2>/dev/null || true)"
