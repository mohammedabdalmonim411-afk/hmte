...
# v1.7: 支持 --worker-id，并行 Worker 隔离 command log
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --worker-id)
            shift
            if [ $# -eq 0 ] || [[ "$1" == --* && "$1" != "--" ]]; then
                error "Missing value for --worker-id"
                exit 1
            fi
            WORKER_ID="$1"
            if ! [[ "$WORKER_ID" =~ ^[-A-Za-z0-9_]{1,64}$ ]]; then
                error "Invalid worker_id: '$WORKER_ID' (must match ^[-A-Za-z0-9_]{1,64}$)"
                exit 1
            fi
            shift
            ;;
        ...
        *)
            if [ "$SEEN_PHASE_ID" = true ]; then
                ...
                error "Usage: hmte exec <phase_id> [--worker-id ID] [--attempt N] -- <command>"
                exit 1
            fi
            PHASE_ID="$1"
            SEEN_PHASE_ID=true
...
# 校验 worker_id（如果提供）不包含路径注入
if [ -n "$WORKER_ID" ]; then
    if [[ "$WORKER_ID" == *"../"* ]]; then
        error "Invalid worker_id: contains '../'"
        exit 1
    fi
    if [[ "$WORKER_ID" == *" "* ]]; then
        error "Invalid worker_id: contains spaces"
        exit 1
    fi
fi
...
if [ ! -d "$SKILL_DIR" ]; then
    error "TAF legacy hmte skill directory not found: $SKILL_DIR"
...
# 2. 准备日志文件名（v1.7: worker_id 隔离）
if [ -n "$WORKER_ID" ]; then
    FILE_STEM="${PHASE_ID}_${WORKER_ID}_attempt_${ATTEMPT}"
else
    FILE_STEM="${PHASE_ID}_attempt_${ATTEMPT}"
fi
LOG_FILE="$LOG_DIR/${FILE_STEM}.commands.jsonl"
...
python3 - "$LOG_FILE" "$PHASE_ID" "$ATTEMPT" "$COMMAND_STR" "$EXIT_CODE" "$STARTED_AT" "$ENDED_AT" "$OUTPUT_FILE" "$WORKER_ID" <<'PY'
import json, sys
from pathlib import Path

log_file, phase_id, attempt, command, exit_code, started_at, ended_at, output_file, worker_id = sys.argv[1:10]
...
if [ $EXIT_CODE -eq 0 ]; then
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        # 采集变更文件列表
        git diff --name-only > "$LOG_DIR/${FILE_STEM}-changed-files.txt" 2>/dev/null || true

        # 采集变更统计
        git diff --stat > "$LOG_DIR/${FILE_STEM}-diff-stat.txt" 2>/dev/null || true
