#!/bin/bash
# Start TAF legacy hmte session with atomic lock
...
echo "Starting TAF legacy hmte runtime..."
...
if ! echo $$ > "$LOCK_FILE" 2>/dev/null; then
    ...
    if [ -f "$LOCK_FILE" ]; then
        ...
        if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
            echo "ERROR: TAF legacy hmte runtime is already running (PID: $OLD_PID)"
            exit 1
        else
...
echo "TAF legacy hmte runtime started successfully"
echo "Project root: $PROJECT_ROOT"
echo "State file: $STATE_FILE"
echo "Lock file: $LOCK_FILE (PID: $$)"
echo ""
echo "To use TAF, invoke the legacy 'hmte' skill in Hermes"
