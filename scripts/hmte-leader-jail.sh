...
# =============================================================================
# hmte-leader-jail.sh — TAF Leader Jail Enforcement
#
...
while IFS= read -r file; do
    ...
    # Command logs are strict JSONL. Other files under logs/ are runtime auxiliaries
    # produced by hmte-exec/final-check and are not command-log evidence.
    if echo "$file" | grep -qE "^\\.phase_control/logs/.*\\.commands\\.jsonl$"; then
        LOG_FILES+=("$file")
        continue
    fi

    if echo "$file" | grep -qE "^\\.phase_control/logs/"; then
        log_pass "Allowed (runtime log artifact): $file"
        continue
    fi
...
for ev_file in evidence_files:
    # Parse phase_id, optional worker_id, and attempt from filename:
    #   sequential: <phase_id>_attempt_<n>.json
    #   shard:      <phase_id>_<worker_id>_attempt_<n>.json
    ...
    # Extract worker_id from evidence JSON (shard-aware)
    try:
        with open(os.path.join(project_root, ev_file)) as f:
            ev_data = json.load(f)
        worker_id = ev_data.get('worker_id', '')
    except Exception:
        worker_id = ''
    if worker_id:
        receipt_path = f".phase_control/delegations/{phase_id}_{worker_id}_attempt_{attempt}_worker.json"
    else:
    ...
    if worker_id:
        log_path = f".phase_control/logs/{phase_id}_{worker_id}_attempt_{attempt}.commands.jsonl"
    else:
...
if project_files:
    ...
    for ev_path in glob.glob(f"{evidence_dir}/*.json"):
        ...
        # Extract worker_id for shard-aware receipt/log path construction
        w_id = ev.get('worker_id', '')
        for cf in ev.get('changed_files', []):
            evidence_index.setdefault(cf, []).append((rel_ev, pid, att, w_id))
        for af in ev.get('artifact_paths', []):
            evidence_index.setdefault(af, []).append((rel_ev, pid, att, w_id))
    ...
    for pf in project_files:
        ...
        chain_failures = []
        for ev_rel, pid, att, w_id in evidence_index.get(pf, []):
            chain_ok = True
            ...
            # 3b. Worker receipt
            if w_id:
                wr_path = f".phase_control/delegations/{pid}_{w_id}_attempt_{att}_worker.json"
            else:
            ...
            if w_id:
                cl_path = f".phase_control/logs/{pid}_{w_id}_attempt_{att}.commands.jsonl"
            else:
...
if [[ -n "$VERDICT_FILES" ]]; then
    while IFS= read -r vfile; do
        ...
        for vf in ${VIOLATIONS[@]+"${VIOLATIONS[@]}"}; do
            if [[ "$vf" == "$vfile" ]]; then
