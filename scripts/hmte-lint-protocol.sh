#!/usr/bin/env bash
# hmte-lint-protocol.sh — TAF 协议检查脚本
...
if not isinstance(phases, list):
    ...
    for i, p in enumerate(phases):
        # Canonical: phase_id
        pid = p.get('phase_id') or p.get('id') or p.get('name')
        if not pid:
            fails.append(f'phase[{i}] 缺 phase_id/id/name')
            continue
        
        # Canonical: phase_id (not id/name)
        if not p.get('phase_id'):
            msg = f'phase[{i}]({pid}) 使用旧字段 id/name（应使用 phase_id）'
            if MODE == 'release':
                fails.append(msg)
            else:
                warns.append(msg)
        
        # Canonical: goal (not objective/description)
        if not p.get('goal'):
            if p.get('objective') or p.get('description'):
                msg = f'phase[{i}]({pid}) 使用旧字段 objective/description（应使用 goal）'
                if MODE == 'release':
                    fails.append(msg)
                else:
                    warns.append(msg)
            else:
                fails.append(f'phase[{i}]({pid}) 缺 goal')
        
        # Canonical: acceptance_criteria (array)
        ac = p.get('acceptance_criteria')
        if not ac:
            fails.append(f'phase[{i}]({pid}) 缺 acceptance_criteria')
        elif not isinstance(ac, list):
            fails.append(f'phase[{i}]({pid}) acceptance_criteria 必须是数组')
...
if not re.match(pattern, fname):
    print(f"FAIL:文件名不符合 {{phase_id}}[_{{worker_id}}]_attempt_{{n}}.json 格式")
else:
...
fname = sys.argv[1]
fpath = sys.argv[2]
# v1.8: Support parallel worker receipt: {phase_id}_{worker_id}_attempt_{n}_{worker|verifier}.json
pattern = r"^[A-Za-z0-9_-]+(_[A-Za-z0-9_-]+)?_attempt_\d+_(worker|verifier)\.json$"
if not re.match(pattern, fname):
    print(f"FAIL:文件名不符合 {{phase_id}}[_{{worker_id}}]_attempt_{{n}}_{{worker|verifier}}.json 格式")
else:
    with open(fpath) as fh:
        data = json.load(fh)
    # v1.8 canonical required: phase_id, attempt, role, delegation_method, delegated_at, timestamp
    required = ["phase_id", "attempt", "role", "delegation_method", "delegated_at", "timestamp"]
    for k in required:
        if k not in data:
            print(f"FAIL:缺必需字段 {k}")
    # trust_level: delegation_trust_level or legacy trust_level (either one required)
    tl = data.get("delegation_trust_level") or data.get("trust_level")
    if not tl:
        print("FAIL:缺 delegation_trust_level 或 trust_level")
    elif tl not in ("INTENT_ONLY", "OBSERVED", "NONE"):
        print(f"FAIL:trust_level={tl} 不合法")
    role = data.get("role")
    if role and role not in ("worker", "verifier"):
        print(f"FAIL:role={role} 不合法，只允许 worker/verifier")
    warns = []
    for k in ["leader_instruction_path", "expected_output_path"]:
        if k not in data:
            warns.append(k)
    if warns:
        print(f"WARN:缺 {chr(44).join(warns)}")
...
valid_orchestrator_worker = bool(re.match(r"^[A-Za-z0-9_-]+_worker_\d+\.json$", fname))
valid_orchestrator_verifier = bool(re.match(r"^[A-Za-z0-9_-]+_verifier_\d+\.json$", fname))
forbidden_patterns = [
    r"_worker_0\.json$",
    r"_verifier_0\.json$",
    r"^instruction_.*\.md$",
...
for pat in forbidden_patterns:
    if re.search(pat, fname):
        ...
        sys.exit(0)
if valid_leader or valid_worker or valid_verifier or valid_orchestrator_worker or valid_orchestrator_verifier:
    print("PASS_FILE")
else:
...
# ═══════════════════════════════════════════════════════════════════
# L11: 规范协议文档存在性检查（legacy .hmte/team-rules.md 仅作兼容，不参与 release 门禁）
# ═══════════════════════════════════════════════════════════════════
check_L11() {
    rule "L11" "protocol 文档存在性检查"
    local protocol="$REPO_ROOT/docs/HTE_PROTOCOL.md"
    local legacy="$REPO_ROOT/.hmte/team-rules.md"
    if [ -f "$protocol" ]; then
        if [ -f "$legacy" ]; then
            pass "L11 docs/HTE_PROTOCOL.md 存在（legacy .hmte/team-rules.md 仅兼容）"
        else
            pass "L11 docs/HTE_PROTOCOL.md 存在"
        fi
        return
    fi
    if [ "$HMTE_LINT_MODE" = "release" ]; then
        fail "L11 docs/HTE_PROTOCOL.md 不存在 (release 模式)"
        inc_fail
    else
        warn "L11 docs/HTE_PROTOCOL.md 不存在 (dev 模式)"
        inc_warn
    fi
}
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo " TAF 协议检查 (mode=$HMTE_LINT_MODE)"
