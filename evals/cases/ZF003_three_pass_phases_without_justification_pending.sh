#!/usr/bin/env bash
#
# ZF003: three_pass_phases_without_justification_pending
#
# Test that gate PENDING when 3+ consecutive phases have zero-finding without justification
#

set -euo pipefail

TEST_NAME="ZF003_three_pass_phases_without_justification_pending"
...
# Create verdict with issue: phase 5, 6, 7 all PASS with empty justification
...
if [ $EXIT_CODE -ne 0 ] && echo "$OUTPUT" | grep -qE "(consecutive.*zero|justification.*required|PENDING|vague language|evidence anchor|timeout.*review)"; then
  echo "✅ PASS: Correctly identified issue for three_pass_phases_without_justification_pending"
  RESULT=0
else
  echo "❌ FAIL: Did not identify issue for three_pass_phases_without_justification_pending"
