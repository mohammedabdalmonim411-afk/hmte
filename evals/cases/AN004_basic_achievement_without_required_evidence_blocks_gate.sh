#!/usr/bin/env bash
#
# AN004: basic_achievement_without_required_evidence_blocks_gate
#
# Test that gate BLOCKS when 'basic achievement' claim lacks required evidence
#

set -euo pipefail

TEST_NAME="AN004_basic_achievement_without_required_evidence_blocks_gate"
...
if [ $EXIT_CODE -ne 0 ] && echo "$OUTPUT" | grep -qE "(basic.*achievement|required.*evidence|vague.*claim)"; then
  echo "✅ PASS: Correctly blocked for basic_achievement_without_required_evidence_blocks_gate"
  RESULT=0
else
  echo "❌ FAIL: Did not block for basic_achievement_without_required_evidence_blocks_gate"
