#!/usr/bin/env bash
#
# ZF009: zero_finding_without_evidence_anchor_fails
#
# Test that gate FAILS when zero-finding justification lacks evidence anchors
#

set -euo pipefail

TEST_NAME="ZF009_zero_finding_without_evidence_anchor_fails"
...
# Create verdict with issue: why_no_p0 has no file/command log/test references
...
if [ $EXIT_CODE -ne 0 ] && echo "$OUTPUT" | grep -qE "(evidence.*anchor|no.*anchor|reference.*required)"; then
  echo "✅ PASS: Correctly identified issue for zero_finding_without_evidence_anchor_fails"
  RESULT=0
else
  echo "❌ FAIL: Did not identify issue for zero_finding_without_evidence_anchor_fails"
