#!/usr/bin/env bash
#
# VM006: verifier_rubber_stamp_pass
#
# Test that gate FAILS when Verifier gives rubber-stamp PASS without review trail
#

set -euo pipefail

TEST_NAME="VM006_verifier_rubber_stamp_pass"
...
# Create Verifier instruction with issue: verdict PASS but review_trail is empty
...
if [ $EXIT_CODE -ne 0 ] && echo "$OUTPUT" | grep -qE "(review.*trail.*required|rubber.*stamp|no.*review)"; then
  echo "✅ PASS: Correctly FAILED for verifier_rubber_stamp_pass"
  RESULT=0
else
  echo "❌ FAIL: Did not FAIL for verifier_rubber_stamp_pass"
