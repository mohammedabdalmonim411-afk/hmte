#!/usr/bin/env bash
#
# VM009: verifier_mandate_omits_changed_file
#
# Test that gate FAILS when Verifier mandate omits changed file from review scope
#

set -euo pipefail

TEST_NAME="VM009_verifier_mandate_omits_changed_file"
...
# Create Verifier instruction with issue: changed_files_to_review missing file from evidence.changed_files
...
if [ $EXIT_CODE -ne 0 ] && echo "$OUTPUT" | grep -qE "(changed.*file.*missing|file.*not.*reviewed|incomplete.*file.*coverage)"; then
  echo "✅ PASS: Correctly FAILED for verifier_mandate_omits_changed_file"
  RESULT=0
else
  echo "❌ FAIL: Did not FAIL for verifier_mandate_omits_changed_file"
