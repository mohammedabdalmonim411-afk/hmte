#!/usr/bin/env bash
#
# AN003: partial_test_pass_without_disposition_blocks_gate
#
# Test that gate BLOCKS when partial test pass has no disposition
#

set -euo pipefail

TEST_NAME="AN003_partial_test_pass_without_disposition_blocks_gate"
...
if [ $EXIT_CODE -ne 0 ] && echo "$OUTPUT" | grep -qE "(partial.*pass.*disposition|disposition.*required|23.*26)"; then
  echo "✅ PASS: Correctly blocked for partial_test_pass_without_disposition_blocks_gate"
  RESULT=0
else
  echo "❌ FAIL: Did not block for partial_test_pass_without_disposition_blocks_gate"
