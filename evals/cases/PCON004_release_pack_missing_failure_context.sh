#!/usr/bin/env bash
#
# PCON004: release_pack_missing_failure_context
#
# Test that release_gate FAILS when release pack omits failure context from history
#

set -euo pipefail

TEST_NAME="PCON004_release_pack_missing_failure_context"
...
{
  "plan_id": "TEST-PCON004",
  "plan_path": "test_plan.md",
  "plan_hash": "sha256:pconpcon004",
  "approved_by": "human",
...
# Create anomaly ledger with contradiction: missing test failures / timeouts from anomaly ledger
cat > "$TEST_DIR/anomaly_ledger.json" <<'EOF'
{
  "ledger_id": "TEST-PCON004-LEDGER",
  "plan_id": "TEST-PCON004",
  "entries": [
    {
      "entry_id": "ANM-001",
...
# Create final report claiming: all checks passed
...
if [ $EXIT_CODE -ne 0 ] && echo "$OUTPUT" | grep -qE "(failure.*context|missing.*context|incomplete.*report)"; then
  echo "✅ PASS: Correctly FAILED for release_pack_missing_failure_context"
  RESULT=0
else
  echo "❌ FAIL: Did not FAIL for release_pack_missing_failure_context"
