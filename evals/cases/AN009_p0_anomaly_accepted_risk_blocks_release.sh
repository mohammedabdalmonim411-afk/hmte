#!/usr/bin/env bash
#
# AN009: p0_anomaly_accepted_risk_blocks_release
#
# Test that release_gate BLOCKS when P0 anomaly is marked as accepted_risk
#

set -euo pipefail

TEST_NAME="AN009_p0_anomaly_accepted_risk_blocks_release"
  ...
  "entries": [
    {
      ...
      "phase_id": "phase_1",
      "anomaly_type": "leader_local_takeover",
...
if [ $EXIT_CODE -ne 0 ] && echo "$OUTPUT" | grep -qE "(P0.*accepted.*risk|P0.*anomaly|critical.*risk)"; then
  echo "✅ PASS: Correctly blocked for p0_anomaly_accepted_risk_blocks_release"
  RESULT=0
else
  echo "❌ FAIL: Did not block for p0_anomaly_accepted_risk_blocks_release"
