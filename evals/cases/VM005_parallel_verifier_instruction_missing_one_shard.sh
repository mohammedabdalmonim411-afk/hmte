#!/usr/bin/env bash
#
# VM005: parallel_verifier_instruction_missing_one_shard
#
# Test that gate FAILS when parallel Verifier instruction omits one shard
#

set -euo pipefail

TEST_NAME="VM005_parallel_verifier_instruction_missing_one_shard"
...
# Create Verifier instruction with issue: audit_plan_ref missing plan item from shard 2
...
if [ $EXIT_CODE -ne 0 ] && echo "$OUTPUT" | grep -qE "(missing.*shard|incomplete.*coverage|plan.*item.*missing)"; then
  echo "✅ PASS: Correctly FAILED for parallel_verifier_instruction_missing_one_shard"
  RESULT=0
else
  echo "❌ FAIL: Did not FAIL for parallel_verifier_instruction_missing_one_shard"
