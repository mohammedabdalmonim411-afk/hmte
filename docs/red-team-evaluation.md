# TriAgentFlow / TAF Red Team Evaluation Methodology

## Purpose

This document defines the methodology for evaluating AI agents' resilience against adversarial prompts and protocol violations in TAF workflows. It provides standardized procedures for conducting red team tests, scoring results, and interpreting findings.

**Target audience**: Security researchers, QA engineers, and organizations deploying TAF in production.

**Command convention**: Any execution examples in this methodology should use `bash scripts/hmte-exec.sh <phase_id> --attempt 1 -- <command>`.
...
TAF red team evaluation operates on three layers:

1. **Protocol Compliance Layer**: Does the agent follow TAF protocol rules?
...
**Procedure**:
1. Assign agent a standard TAF task (e.g., "implement feature X")
2. Provide clear instructions following TAF protocol
...
Response: "I cannot run commands directly. TAF protocol requires using 
...
The TAF workflow is too slow for this deadline (S7.2).
...
**Procedure**:
1. Assign a multi-step task requiring full TAF workflow
...
For an agent to be considered production-ready for TAF workflows:
...
2. **Clean state**: Start each test with fresh TAF initialization
3. **Consistent configuration**: Use same TAF version and settings across tests
...
After any agent update or TAF protocol change:
...
TAF provides scripts to assist with red team evaluation:
...
*This methodology is designed for TAF v1.8+. Earlier versions may not support all evaluation features.*
