# TriAgentFlow / TAF Attack Cases and Detection Boundaries

> Historical examples may use the former HTE name; the current public name is TriAgentFlow / TAF.

## Purpose

This document catalogs known attack vectors and forgery paths in the TriAgentFlow / TAF workflow. It serves as an honest assessment of what TAF can detect versus what it cannot detect, helping users understand the current boundaries of the verification system.

**Key principle**: TAF is designed to verify that AI agents follow proper delegation and verification protocols. However, it cannot prevent all forms of manipulation, especially those involving human actors or coordinated deception across multiple roles.
...
### Current Limitations
TAF relies on the Verifier's integrity. It can verify that verification *happened* (via exec logs), but cannot verify that the verification was *thorough* or *honest*.
...
### Attack Description
A Worker or Verifier runs commands directly in the shell instead of through `hmte exec`, avoiding the creation of execution logs that TAF uses for verification.
...
### Current Limitations
TAF can only verify what is claimed. If an agent performs work but doesn't document it in receipts, TAF has no way to detect the bypass. The system assumes agents will honestly report their actions.
...
### Current Limitations
TAF v1.4 has limited support for verifying OBSERVED delegation. The protocol requires tool-call traces, but validating their authenticity is challenging. Future versions may add cryptographic signatures or timestamps to strengthen this.
...
### Current Limitations
TAF can verify that evidence came from `hmte exec` by checking for corresponding logs, but it cannot verify that all relevant evidence was included or that the evidence wasn't cherry-picked.
...
### What TAF Can Detect
- **Identical agent IDs**: If agent identifiers are tracked, TAF can detect when the same agent appears in multiple roles
...
### Current Limitations
TAF v1.4 does not currently track agent identities across roles. This is a known limitation. The protocol assumes organizational policies will prevent self-verification, but TAF itself doesn't enforce this separation.

**Recommendation**: Organizations should implement external agent identity tracking and enforce role separation policies outside of TAF.
...
### Current Limitations
This is a meta-vulnerability affecting TAF's own development. The solution requires:
...
**Current status**: TAF v1.8 includes `hmte-lint-protocol.sh` which checks many protocol rules, but not all documented requirements are enforced. This document itself helps identify the gap.
...
### Attack Description
End-to-end tests create valid-looking TAF artifacts (receipts, evidence, verdicts) programmatically, but these files don't represent actual AI agent execution. An attacker could use similar techniques to forge an entire workflow.
...
- **Test vs. production**: TAF cannot distinguish between legitimate E2E tests and malicious forgeries

### Current Limitations
This is a fundamental limitation of file-based verification. TAF verifies that artifacts follow the protocol, but cannot verify that they represent genuine AI agent execution.
...
**Current status**: TAF v1.8 is designed for honest agents in cooperative environments. It's a protocol compliance tool, not a security system. Organizations requiring stronger guarantees should implement additional controls.

---

## Attack Vector 8: Fake Completion Report

**Chinese**: 伪造完成报告 (fake completion report claiming "Phase PASS" without following TAF workflow)

### Attack Description
An Agent directly modifies files and outputs fake "Phase PASS" completion tables, claiming that evidence and verdicts have been written, but without actually following the TAF workflow. The agent bypasses the proper phase workflow (no kickoff, no Worker/Verifier loop, no phase_gate execution) and instead fabricates completion reports in natural language.
...
### What TAF Cannot Detect
- **Natural language deception**: If the user doesn't run `hmte-final-check.sh`, TAF has no way to prevent the agent from claiming completion in conversational output
...
### Current Limitations
TAF's phase control system can verify workflow compliance when `hmte-final-check.sh` is executed, but it cannot prevent an agent from making false claims in natural language responses. The system relies on users running the final check script to validate that all phases were properly executed.
...
**Current status**: TAF v1.8 includes `hmte-final-check.sh` which validates that all phases followed proper workflow, but this check must be manually invoked by the user.
...
TAF is designed with the following assumptions:
...
TriAgentFlow / TAF is **not** designed to:
...
1. **Use TAF as a compliance tool**, not a security boundary
...
5. **Combine TAF with other controls** (code review, testing, monitoring)
6. **Report gaps** between documentation and enforcement to improve TAF
...
*This document was originally introduced in v1.4 and is maintained as a living threat model. Current capabilities reflect TAF v1.8 (June 2026).*
